...
/**
 * /INITIATIVE - Set initiative modifier (-10 to +10)
...
public class InitiativeCommand extends AbstractPlayerCommand {
    ...
    public InitiativeCommand(GridMoveManager playerManager, RoleManager roleManager, EncounterManager encounterManager) {
        super("INITIATIVE", "Set initiative modifier (-10 to +10)");
        this.playerManager = playerManager;
        this.roleManager = roleManager;
        this.encounterManager = encounterManager;
        this.valueArg = this.withRequiredArg("value", "Initiative modifier (-10 to +10)", ArgTypes.INTEGER);
    }
    ...
    protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store,
        ...
        if (value < -10 || value > 10) {
            Message primary = Message.raw("Initiative must be between -10 and +10!").color("#FF0000");
        ...
        if (roleManager.isGM(playerRef)) {
            ...
            monster.stats.initiative = value;

            Message primary = Message.raw(monster.getDisplayName() + " - Initiative").color("#FFD700");
            Message secondary = Message.raw(sign + value).color("#FFFFFF");
            ...
            state.stats.initiative = value;

            Message primary = Message.raw("Initiative").color("#FFD700");
            Message secondary = Message.raw(sign + value).color("#FFFFFF");
            ItemWithAllMetadata icon = new ItemStack("Ingredient_Crystal_Yellow", 1).toPacket();
